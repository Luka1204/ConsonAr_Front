...
//#region node_modules/pitchfinder/lib/detectors/yin.js
var require_yin = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	function YIN(params) {
		...
		return function YINDetector(float32AudioBuffer) {
			...
			for (var t = 0; t < yinBufferLength; t++) yinBuffer[t] = 0;
			for (var t = 1; t < yinBufferLength; t++) for (var i = 0; i < yinBufferLength; i++) {
			...
			for (tau = 2; tau < yinBufferLength; tau++) if (yinBuffer[tau] < threshold) {
			...
			*/
			...
			if (x0 === tau) if (yinBuffer[tau] <= yinBuffer[x2]) betterTau = tau;
			else betterTau = x2;
			else if (x2 === tau) if (yinBuffer[tau] <= yinBuffer[x0]) betterTau = tau;
			else betterTau = x0;
			else {
				var s0 = yinBuffer[x0];
...
//#endregion
//#region node_modules/pitchfinder/lib/detectors/amdf.js
var require_amdf = /* @__PURE__ */ __commonJSMin(((exports) => {
	var __assign = exports && exports.__assign || function() {
		__assign = Object.assign || function(t) {
			for (var s, i = 1, n = arguments.length; i < n; i++) {
				s = arguments[i];
	...
	function AMDF(params) {
		...
		return function AMDFDetector(float32AudioBuffer) {
			...
			for (i = 0; i < maxShift; i++) if (minPeriod <= i && i <= maxPeriod) {
				...
				calcSub = [];
				for (u = 0; u < frameLength; u++) calcSub[u] = frames1[u] - frames2[u];
				var summation = 0;
				for (u = 0; u < frameLength; u++) summation += Math.abs(calcSub[u]);
				amd[i] = summation;
			}
			...
			minval = amd[j];
			var minpos = j;
			for (i = j - 1; i < j + searchLength && i <= maxPeriod; i++) if (amd[i] < minval) {
				minval = amd[i];
				minpos = i;
			}
			if (Math.round(amd[minpos] * ratio) < maxval) return sampleRate / minpos;
			else return null;
		};
	}
	exports.AMDF = AMDF;
...
//#endregion
//#region node_modules/pitchfinder/lib/detectors/acf2plus.js
var require_acf2plus = /* @__PURE__ */ __commonJSMin(((exports) => {
	var __assign = exports && exports.__assign || function() {
		__assign = Object.assign || function(t) {
			for (var s, i = 1, n = arguments.length; i < n; i++) {
				s = arguments[i];
	...
	Object.defineProperty(exports, "__esModule", { value: true });
	var DEFAULT_PARAMS = { sampleRate: 44100 };
	function ACF2PLUS(params) {
		if (params === void 0) params = DEFAULT_PARAMS;
		var sampleRate = __assign(__assign({}, DEFAULT_PARAMS), params).sampleRate;
		return function ACF2PLUSDetector(float32AudioBuffer) {
			...
			var aux2 = maxShift - 1;
			var thres = .2;
			for (i = 0; i < maxShift / 2; i++) if (Math.abs(float32AudioBuffer[i]) < thres) {
				aux1 = i;
				break;
			}
			for (i = 1; i < maxShift / 2; i++) if (Math.abs(float32AudioBuffer[maxShift - i]) < thres) {
				aux2 = maxShift - i;
				break;
			}
			...
			for (i = 0; i < framesLength; i++) for (j = 0; j < framesLength - i; j++) calcSub[i] = calcSub[i] + frames[j] * frames[j + i];
			u = 0;
			while (calcSub[u] > calcSub[u + 1]) u++;
			var maxval = -1, maxpos = -1;
			for (i = u; i < framesLength; i++) if (calcSub[i] > maxval) {
				maxval = calcSub[i];
				maxpos = i;
			}
			var T0 = maxpos;
...
//#endregion
//#region node_modules/pitchfinder/lib/detectors/dynamic_wavelet.js
var require_dynamic_wavelet = /* @__PURE__ */ __commonJSMin(((exports) => {
	var __assign = exports && exports.__assign || function() {
		__assign = Object.assign || function(t) {
			for (var s, i = 1, n = arguments.length; i < n; i++) {
				s = arguments[i];
	...
	var MAXIMA_THRESHOLD_RATIO = .75;
	var DEFAULT_DYNAMIC_WAVELET_CONFIG = { sampleRate: 44100 };
	function DynamicWavelet(params) {
		if (params === void 0) params = {};
		var sampleRate = __assign(__assign({}, DEFAULT_DYNAMIC_WAVELET_CONFIG), params).sampleRate;
		return function DynamicWaveletDetector(float32AudioBuffer) {
			...
			minValue -= theDC;
			maxValue -= theDC;
			var amplitudeThreshold = (maxValue > -1 * minValue ? maxValue : -1 * minValue) * MAXIMA_THRESHOLD_RATIO;
			var curLevel = 0;
			...
			while (true) {
				...
				var d = void 0;
				var distances = [];
				for (var i = 0; i < curSamNb; i++) distances[i] = 0;
				for (var i = 0; i < nbMins; i++) for (var j = 1; j < DIFFERENCE_LEVELS_N; j++) if (i + j < nbMins) {
				...
				for (var i = 0; i < curSamNb; i++) {
					var summed = 0;
					for (var j = -1 * delta; j <= delta; j++) if (i + j >= 0 && i + j < curSamNb) summed += distances[i + j];
					if (summed === bestValue) {
				...
				var distAvg = 0;
				var nbDists = 0;
				for (var j = -delta; j <= delta; j++) if (bestDistance + j >= 0 && bestDistance + j < bufferLength) {
				...
				curLevel++;
				if (curLevel >= MAX_FLWT_LEVELS || curSamNb < 2) break;
				var newFloat32AudioBuffer = float32AudioBuffer.subarray(0);
				if (curSamNb === distances.length) newFloat32AudioBuffer = new Float32Array(curSamNb / 2);
				for (var i = 0; i < curSamNb / 2; i++) newFloat32AudioBuffer[i] = (float32AudioBuffer[2 * i] + float32AudioBuffer[2 * i + 1]) / 2;
...
//#endregion
//#region node_modules/pitchfinder/lib/detectors/macleod.js
var require_macleod = /* @__PURE__ */ __commonJSMin(((exports) => {
	var __assign = exports && exports.__assign || function() {
		__assign = Object.assign || function(t) {
			for (var s, i = 1, n = arguments.length; i < n; i++) {
				s = arguments[i];
	...
	function Macleod(params) {
		...
		*/
		function normalizedSquareDifference(float32AudioBuffer) {
			...
			for (var i = 1; i < float32AudioBuffer.length; i += 1) squaredBufferSum[i] = float32AudioBuffer[i] * float32AudioBuffer[i] + squaredBufferSum[i - 1];
			for (var tau = 0; tau < float32AudioBuffer.length; tau++) {
				...
				for (var i = 0; i < float32AudioBuffer.length - tau; i++) acf += float32AudioBuffer[i] * float32AudioBuffer[i + tau];
		...
		*/
		...
		return function Macleod(float32AudioBuffer) {
			...
			if (periodEstimates.length) {
				...
				var periodIndex = 0;
				for (var i = 0; i < ampEstimates.length; i++) if (ampEstimates[i] >= actualCutoff) {
					periodIndex = i;
					break;
				}
				var pitchEstimate = sampleRate / periodEstimates[periodIndex];
				if (pitchEstimate > LOWER_PITCH_CUTOFF) pitch = pitchEstimate;
				else pitch = -1;
			} else pitch = -1;
			return {
...
//#endregion
//#region node_modules/pitchfinder/lib/tools/frequencies.js
var require_frequencies = /* @__PURE__ */ __commonJSMin(((exports) => {
	var __assign = exports && exports.__assign || function() {
		__assign = Object.assign || function(t) {
			for (var s, i = 1, n = arguments.length; i < n; i++) {
				s = arguments[i];
	...
	function frequencies(detector, float32AudioBuffer, options) {
		...
		var getPitch;
		if (Array.isArray(detector)) getPitch = pitchConsensus.bind(null, detector);
		else getPitch = detector;
		var pitches = [];
...
//#endregion
//#region node_modules/pitchfinder/lib/index.js
